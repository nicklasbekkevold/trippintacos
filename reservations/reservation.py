...
def get_next_available_table(restaurant, reservation_date_time, number_of_people, minutes_slot=120):
    ...
    tables_booked = Reservation.objects.filter(table_restaurant=restaurant,
                                               reservation_date_time_start__lt=lower_bound_time,
                                               reservation_date_time_end__gt=lower_bound_time).values('table')
    ...
    tables_booked = Reservation.objects.filter(reservation_date_time_start_lt=upper_bound_time,
                                               reservation_date_time_end__gt=upper_bound_time).values('table')
    tables_booked_ids_temp = [x['table'] for x in tables_booked]
    tables_booked_ids = tables_booked_ids + tables_booked_ids_temp

    # ekskluderer bookede bord som er inni den aktuelle tidsperioden. er dette jalla?

    tables_booked = Reservation.objects.filter(reservation_date_time_start__gt=lower_bound_time,
                                               reservation_date_time_end__lt=upper_bound_time).values('table')
    tables_booked_ids_temp = [x['table'] for x in tables_booked]
    tables_booked_ids = tables_booked_ids + tables_booked_ids_temp

    # ekskluderer bord som inkluderer den etterspurte booking-luken

    tables_booked = Reservation.objects.filter(booking_date_time_start__lt=lower_bound_time,
                                               booking_date_time_end__gt=upper_bound_time).values('table')
    tables_booked_ids_temp = [x['table'] for x in tables_booked]
    tables_booked_ids = tables_booked_ids + tables_booked_ids_temp
    ...
    tables = Table.objects.filter(restaurant=restaurant, restaurant__opening_time_lte=reservation_date_time.hour,
                                  restaurant__closing_time__gte=reservation_date_time.hour + (minutes_slot / float(60)),
                                  size__gte=number_of_people).exclude(id__in=tables_booked_ids).order_by('size')
...
def make_reservation(restaurant, guest, reservation_date_time, number_of_people, minutes_slot=120, ):
    ...
    if table:
        ...
        reservation = Reservation(guest=guest, number_of_people=number_of_people, start_date_time=reservation_date_time,
                                  end_date_time=reservation_date_time + delta, createdDate=datetime.now(), table=table)
        reservation.save()
