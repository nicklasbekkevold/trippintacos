...
def get_available_times_v2(numberOfPeople:int, startDate:str):
    ...
    datetime_start = datetime(year, month, day, 12, 0, 0, 0, pytz.UTC)
    datetime_end = datetime(year, month, day, 23, 59, 59, 59, pytz.UTC)
    for res in Reservation.objects.all():
        try:
            print("Hei")
            res.start_date_time = pytz.utc.localize(res.start_date_time)
        except:
            print("except")
            continue
    ...
    print("RESERVATIONS THIS DATE: ", reservations_this_date)
    available_times = list()

    tables = Table.objects.filter(number_of_seats__gte=numberOfPeople)

    datetime_counter = datetime(year, month, day, 12, 0, 0, 0, pytz.UTC)
    coll = False
    '''
    while datetime_counter.replace(tzinfo=None) <= datetime(year, month, day, 22).replace(tzinfo=None):
        for table in tables:
            reservation_at_table = reservations_this_date.filter(table_id=table.id)
            for res in reservation_at_table:
                if helpers.checkForCollision(datetime_counter, datetime_counter + timedelta(hours=2), res):
                    coll = True
            ...
            if not coll:
                if str(datetime_counter.minute) == '30':
                    datetimeTemp = str((datetime_counter + timedelta(hours=1)).hour) + ":30"
                else:
                    datetimeTemp = str((datetime_counter + timedelta(hours=1)).hour) + ":00"
                times_set_v2.add((datetimeTemp, datetimeTemp))
                coll = False
        datetime_counter += timedelta(minutes=30)

    return sorted(list(times_set_v2), key=lambda x: x[0])
    '''
    while datetime_counter <= datetime(year, month, day, 22, 0, 0, 0, pytz.UTC):
        coll = False
        for _table in tables:
            ...
            for reservation in QS_reservations_at_date_at_table:
                print("Res start:", reservation.start_date_time)
                if helpers.checkForCollision(datetime_counter, datetime_counter + timedelta(hours=2), reservation):
                    print("Collision")
                    coll = True
                    continue  # if there is a collision, we break and go on to a new table
            if not coll:
                print("Not coll ", datetime_counter, "Table", _table)
                if str(datetime_counter.minute) == '30':
